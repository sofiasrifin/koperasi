...
{
$key= htmlEntities($t_inbox[TextDecoded], ENT_QUOTES);
$pieces = explode(" ", $key);
...
if($j_auto<>0)
{
		...
		// pesan asli
		$pesan = "$headauto$t_autorespond[isi_sms]$footauto";
		...
		if($jmlSMS1==1)
		{
				...
				('$t_inbox[SenderNumber]', '$pesan', '$newID', 'true', '$t_inbox[RecipientID]', 'Gammu')");
		}
		if($jmlSMS1<>1)
		{
				...
				for ($i=1; $i<=$jmlSMS; $i++)
				{
					...
					if ($i == 1)
					{
          				...
          				VALUES ('$t_inbox[SenderNumber]', '$udh', '$msg', '$newID', 'true', '$t_inbox[RecipientID]', 'Gammu')";
					}
					else
   					{
	...
	$jmlSMS1 = ceil(strlen($pesan)/160);
		...
		for ($i=1; $i<=$jmlSMS; $i++)
		{
   		...
   		if ($i == 1)
            ...
            VALUES ('$tm_jdw[no_hp]', '$udh', '$msg', '$newID', 'true', '$tm_jdw[SenderID]', 'Gammu','$tm_jdw[Class]')";
   			}
   		else
   			{
      		// jika bukan merupakan pecahan pertama, simpan ke tabel OUTBOX_MULTIPART
...
$sentitem=mysql_query("select DestinationNumber,TextDecoded,ID,SenderID,Status from sentitems");
while($tmp_sent=mysql_fetch_array($sentitem))
{
	$status=$tmp_sent[Status];
	if($status=='SendingError' or $status=='DeliveryFailed')
	{
	$stat_schedule = mysql_query("INSERT INTO outbox (DestinationNumber, TextDecoded, ID, MultiPart, SenderID, CreatorID) 
	VALUES ('$tmp_sent[DestinationNumber]', '$tmp_sent[TextDecoded]', '$newID', 'true', '$tmp_sent[SenderID]', 'Gammu')");
...
<audio controls="controls" autoplay="autoplay">
  <source src="sound_sms/<?php echo $t_sound[nama_file];?>" type="audio/mpeg" />
<embed src="sound_sms/<?php echo $t_sound[nama_file];?>" />
</audio>
</div>
<?php 
}
