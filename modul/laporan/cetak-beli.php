...
$where	= " WHERE a.kodebeli ='$kode'";
$q	= mysql_query("SELECT * FROM h_beli WHERE kodebeli='$kode'");
$d 	= mysql_fetch_array($q);
$tgl 	= jin_date_str($d[tglbeli]);
...
$query = "SELECT a.*,b.*,c.nama_barang 
		...
		join d_beli as b
		JOIN barang as c
		on a.kodebeli=b.kodebeli and b.kode_barang=c.kode_barang
		$where
		order by idbeli";
...
function myfooter(){	
	...
	echo "<tr>
			...
			<td align='left'>$r_data[jmlbeli]</td>
<!--		<td align='center'>$r_data[satuan]</td>		-->
			<td align='right'>".number_format($r_data[hargabeli])."</td>
