...
function cari_jml_beli($kode){
	$sql	= "SELECT kode_barang,sum(jmlbeli) as jml FROM d_beli WHERE kode_barang='$kode'";
	$query	= mysql_query($sql);
	$row	= mysql_num_rows($query);
	if ($row>0){
		$data	= mysql_fetch_array($query);
...
function cari_jml_jual($kode){
	$sql	= "SELECT kode_barang,sum(jmljual) as jml FROM d_jual WHERE kode_barang='$kode'";
	$query	= mysql_query($sql);
	$row	= mysql_num_rows($query);
	if ($row>0){
		$data	= mysql_fetch_array($query);
...
function jml_beli($kode){
	$sql	= "SELECT kodebeli,count(*) as jml FROM d_beli WHERE kodebeli='$kode'";
	$query	= mysql_query($sql);
	$row	= mysql_num_rows($query);
	if ($row>0){
		$data	= mysql_fetch_array($query);
